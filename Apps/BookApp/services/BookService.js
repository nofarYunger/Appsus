...
function _createBooks() {
    var books = StorageService.load(KEY)
    if (!books || !books.length) books = getBooks()
    gBooks = books
    StorageService.save(KEY, gBooks)
}

function addBookToDB(book) {
    const newBookDit = book.volumeInfo
    var newBook = {
        title: newBookDit.title,
        isOnSale: true,
        id: UtilService.makeId(),
        authors: newBookDit.authors,
        pageCount: newBookDit.pageCount,
        listPrice: { amount: UtilService.getRandomInt(10, 200) },
        description: newBookDit.description,
        thumbnail: newBookDit.imageLinks.thumbnail
    }
...
function getById(bookId) {
    const _book = gBooks.find(book => book.id === bookId)
    return Promise.resolve(_book)
}

function addReview(rev, bookId) {
    var copyBooks = [...gBooks]
    const idx = copyBooks.findIndex(book => book.id === bookId)
    let reviews = copyBooks[idx].reviews
    if (!reviews) copyBooks[idx].reviews = []
    copyBooks[idx].reviews.push(rev)
    gBooks = copyBooks
    StorageService.save(KEY, gBooks)
    return Promise.resolve()
}
